 ...
 ** cal_D2TS_bearing
 **
 ** Calculate the distance (GT_D2TS) & Vehical Bearing angle to TS
 ** Using IMU_Lat_lon and GT_Lat_lon of TS
 */
int cal_D2TS_bearing(app_struct* ptr_struct_obj)
{
	// Packet structure define
	PACKET* ptr_metadata = (PACKET*) & (ptr_struct_obj->stPacket);
	...
	// Distance between the two GPS points
	float GT_d2TS = sqrt(pow(sy, 2) + pow(sx, 2));

	ptr_gtMetadata->f4_gt_distance_TS = GT_d2TS;
	...
	ptr_metadata->f4_ins_bearing_TS = bearingAngle;
