...
int accept_client(netrx* ptrCliNet)
{
	...
	ptrCliNet->sock_output = Application_socket;
...
int send_packet(netrx* ptrclient)
{
	...
	send_size = send(ptrclient->sock_output, (char*)ptr_metadata, sizeof(PACKET), 0);
	if (send_size == -1) {
...
int recv_packet(netrx* ptrclient)
{

	int read_size;
	PACKET* ptr_metadata;
	ptr_metadata = (PACKET*) & (ptrclient->stPacket);
	read_size = recv(ptrclient->sock_output, (char*)ptr_metadata, sizeof(PACKET), 0);
	if (read_size < 0) {
